...
type SessionTokenPayload = LocalAuthSession & {
  iat: number;
  exp: number;
};
...
function getSessionSecret() {
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "AUTH_SESSION_SECRET (or AUTH0_SECRET) is required in production",
    );
  }
...
function serializeSession(session: LocalAuthSession) {
...
function parseSession(value?: string | null): LocalAuthSession | null {
